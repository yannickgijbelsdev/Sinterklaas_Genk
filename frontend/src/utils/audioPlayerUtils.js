// Utility functions for custom wave audio player - NO DOM MANIPULATION
export const initializeAudioPlayers = () => {
  // Only initialize existing wave-audio-container elements, don't convert anything
...
// Create a more aggressive approach to replace audio elements
const aggressiveAudioReplacement = () => {
  // Look for any element containing audio with filename
  const replaceAudioInElement = (element) => {
    const textContent = element.textContent || '';
    if (textContent.includes('.mp3') && textContent.includes('🎵')) {
      const audioElement = element.querySelector('audio');
      if (audioElement) {
        const src = audioElement.src || (audioElement.querySelector('source') ? audioElement.querySelector('source').src : '');
        if (src) {
          // Replace entire container with wave player
          ...
          `;
          element.outerHTML = wavePlayerHTML;
          return true;
        }
      }
    }
    ...
    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        if (node.nodeType === Node.ELEMENT_NODE) {
          if (replaceAudioInElement(node)) {
            // If we replaced something, re-initialize
            setTimeout(initializeAudioPlayers, 100);
          }
          // Also check child elements
          node.querySelectorAll && node.querySelectorAll('div').forEach(div => {
            if (!div.classList.contains('wave-audio-container')) {
              if (replaceAudioInElement(div)) {
                setTimeout(initializeAudioPlayers, 100);
              }
            }
          });
...
export default { initializeAudioPlayers, aggressiveAudioReplacement };